...
class RoomManager {
  ...
  public UpdateHost(host: WebSocketClient, roomId?: string, secret?: string): [boolean, string] {
    if (!roomId) {
  ...
  public RemoveHost(roomId?: string, secret?: string): [boolean, string] {
    if (!roomId) {
      return [false, "Missing required roomID field."];
    }
    const roomInfo = this.roomInfo.get(roomId);
  ...
  public AddClient(client: WebSocketClient, secret: string, roomId?: string, name?: string): [boolean, string] {
    if (!roomId) {
      return [false, "Missing required roomID field."];
    }
    const roomInfo = this.roomInfo.get(roomId);
  ...
  public UpdateClient(client: WebSocketClient, roomId?: string, name?: string, secret?: string): [boolean, string] {
    if (!roomId) {
      return [false, "Missing required roomID field."];
    }
    const roomInfo = this.roomInfo.get(roomId);
...
function ClientShareAbility(host: Peer.Instance, ability: string, isStrength?: boolean) {
  const json = {
    type: "ShareAbility",
    fields: {
      role: Role.Client,
